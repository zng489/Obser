...
def __find_download_link(base_value, **kwargs):
    res = requests.get('https://www.gov.br/inep/pt-br/acesso-a-informacao/dados-abertos/indicadores-educacionais/media-de-alunos-por-turma')
    ...
    for div_all_tx in parent_all_tx_divs[::-1]:
        ...
        parent_year_tx = body.find(name='div', attrs={'id': 'parent-fieldname-text'})
...
def main(**kwargs):
    ...
    key_name = 'org_raw_inep_atu'
    tmp = '/tmp/org_raw_inep_atu/'
    ...
    try:
        ...
        for year, urls in __find_download_link(base_value, **kwargs):
            for table_all, url in urls:
                ...
                __drop_directory(adl, schema='inep_atu', table=table_all, year=year)
                for file in files:
                    ...
                    __upload_file(adl, schema='inep_atu', table=table_all, year=year, file=parquet_output)
