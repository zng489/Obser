...
def __find_download_link(base_value, **kwargs):
    res = requests.get('https://www.gov.br/inep/pt-br/acesso-a-informacao/dados-abertos/indicadores-educacionais/taxas-de-distorcao-idade-serie')
...
def main(**kwargs):
    ...
    key_name = 'org_raw_inep_tdi'
    tmp = '/tmp/org_raw_inep_tdi/'
    ...
    try:
        ...
        for year, urls in __find_download_link(base_value, **kwargs):
            for table_all, url in urls:
                ...
                __drop_directory(adl, schema='inep_tdi', table=table_all, year=year)
                for file in files:
                    ...
                    __upload_file(adl, schema='inep_tdi', table=table_all, year=year, file=parquet_output)
