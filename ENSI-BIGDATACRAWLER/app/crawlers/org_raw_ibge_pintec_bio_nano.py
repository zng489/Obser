...
def __fix_column_names(df):
    name_conversion = {
        'EMPRESAS_EMPRESAS_QUE_REALIZARAM_ATIVIDADES_EM_EM_NANOTECNOLOGIA_TOTAL_POR_MODO_DE_USO_P&D':'EMPRESAS_QUE_REALIZARAM_ATIVIDADES_EM_NANOTECNOLOGIA_TOTAL',
        'EMPRESAS_EMPRESAS_QUE_REALIZARAM_ATIVIDADES_EM_EM_NANOTECNOLOGIA_INOVADORAS_TOTAL_P&D':'EMPRESAS_QUE_REALIZARAM_ATIVIDADES_EM_NANOTECNOLOGIA_INOVADORAS_TOTAL'
    }

    df.columns = [name_conversion[column] if column in name_conversion.keys() else column.replace('_EM_EM_', '_EM_').replace('EMPRESAS_EMPRESAS_', 'EMPRESAS_') for column in df.columns]
...
def main(**kwargs):
    ...
    tmp = '/tmp/org_raw_pintec_bio_nano/'
    table = 'pintec_bio_nano'
