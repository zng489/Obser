...
def __find_download_link_escola(base_value, **kwargs):
    res = requests.get('https://www.gov.br/inep/pt-br/acesso-a-informacao/dados-abertos/indicadores-educacionais/complexidade-de-gestao-da-escola')
    ...
    try:
        ...
        for year, urls in __find_download_link_escola(base_value, **kwargs):
            for table, url in urls:
                ...
                __drop_directory(adl, schema='icg', table=table, year=year)
                df = pd.concat(__parse_xlsx(files), axis='index', ignore_index=True)

                parquet_output = tmp + '{file}.parquet'.format(file=table)
                df.to_parquet(parquet_output, index=False)
                __upload_file(adl, schema='inep_icg', table=table, year=year, file=parquet_output)
