...
def __find_download_link(base_value, **kwargs):
    res = requests.get('https://www.gov.br/inep/pt-br/acesso-a-informacao/dados-abertos/indicadores-educacionais/percentual-de-docentes-com-curso-superior', timeout=60)
    ...
    for div_all_tx in parent_all_tx_divs[::-1]:
        ...
        soup = bs4.BeautifulSoup(res.text, features='html.parser')
        parent_year_tx = soup.find(name='div', attrs={'data-nav': 'percentual-de-docentes-com-curso-superior'})
...
def __parse_xlsx(file):
    ...
    for sheet in workbook.sheet_names():
        ...
        _df = pd.read_excel(file, sheet_name=sheet, header=None, names=header_first, index_col=False, skiprows=skip_header)
...
def main(**kwargs):
    ...
    key_name = 'org_raw_inep_dsu'
    tmp = '/tmp/org_raw_inep_dsu/'
    ...
    try:
        ...
        for year, urls in __find_download_link(base_value, **kwargs):
            for table_all, url in urls:
                ...
                __drop_directory(adl, schema='inep_dsu', table=table_all, year=year)
                for file in files:
                    ...
                    __upload_file(adl, schema='inep_dsu', table=table_all, year=year, file=parquet_output)
