...
def main(**kwargs):
    ...
    tmp = '/tmp/org_raw_pop_estimada/'
    table = 'pop_estimada'
    try:
        os.makedirs(tmp, mode=0o777, exist_ok=True)

        current_year = datetime.datetime.now().year
        years = range(2014, current_year+1, 1)

        url_base = f'https://apisidra.ibge.gov.br/values/t/6579/n6/all/v/all/p/'
        ...
        for year in years:
            ...
            if response.status_code != 200:
                raise Exception(f'status_code not 200. Server message: Code {response.status_code} | {response.text}')
