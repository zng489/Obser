...
def main(**kwargs):
    ...
    key_name = 'org_raw_ms_sinan_pneu'
    tmp = '/tmp/org_raw_ms_sinan_pneu/'
    ...
    try:
        ...
        for  year in years:
            ...
            if len(response.json())>0:
                ...
                __drop_directory(adl, schema='ms_sinan', table='pneu', year=year)

                parquet_output = tmp + '{file}.parquet'.format(file=os.path.basename(output))
                df.to_parquet(parquet_output, index=False)

                __upload_file(adl, schema='ms_sinan', table='pneu', year=year, file=parquet_output)
