...
def __find_download_link(base_value, **kwargs):
    res = requests.get('https://www.gov.br/inep/pt-br/acesso-a-informacao/dados-abertos/indicadores-educacionais/media-de-horas-aula-diaria')
    ...
    for div_all_tx in parent_all_tx_divs[::-1]:
        ...
        soup = bs4.BeautifulSoup(res.text, features='html.parser')
        parent_year_tx = soup.find(name='div', attrs={'data-nav': 'média-de-horas-aula-diaria'})
...
def main(**kwargs):
    ...
    key_name = 'org_raw_inep_had'
    tmp = '/tmp/org_raw_inep_had/'
    ...
    try:
        ...
        for year, urls in __find_download_link(base_value, **kwargs):
            for table_all, url in urls:
                ...
                __drop_directory(adl, schema='inep_had', table=table_all, year=year)
                for file in files:
                    ...
                    __upload_file(adl, schema='inep_had', table=table_all, year=year, file=parquet_output)
