...
def __find_download_link(base_value, **kwargs):
    res = requests.get('https://www.gov.br/inep/pt-br/acesso-a-informacao/dados-abertos/indicadores-educacionais/remuneracao-media-dos-docentes', timeout=60)
    ...
    for div_all_tx in parent_all_tx_divs[::-1]:
        ...
        soup = bs4.BeautifulSoup(res.text, features='html.parser')
        parent_year_tx = soup.find(name='div', attrs={'data-nav': 'remuneracao-média-dos-docentes'})
...
def main(**kwargs):
    ...
    key_name = 'org_raw_inep_rmd'
    tmp = '/tmp/org_raw_inep_rmd/'
    ...
    try:
        ...
        for year, urls in __find_download_link(base_value, **kwargs):
            for table_all, url in urls:
                ...
                __drop_directory(adl, schema='inep_rmd', table=table_all, year=year)
                for file in files:
                    ...
                    __upload_file(adl, schema='inep_rmd', table=table_all, year=year, file=parquet_output)
